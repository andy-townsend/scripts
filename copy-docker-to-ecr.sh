...
def mirror_docker_hub_images_to_ecr(ecr_client, *, account_id, image_tags):
    ...
    for hub_tag in image_tags:
        ecr_tag = f"{account_id}.dkr.ecr.eu-west-2.amazonaws.com/{hub_tag}"
...
if __name__ == "__main__":
    mirror_docker_hub_images_to_ecr(
        ecr_client=boto3.client("ecr","eu-west-2"), account_id=ACCOUNT_ID, image_tags=IMAGE_TAGS
    )
